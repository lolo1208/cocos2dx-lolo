...
def main():
    ...
    ### --------[ changed ]-------- ###   cpp_root : runtime-src/Classes/
    cpp_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    config.set('DEFAULT', 'cpp_root', cpp_root)
    ### --------[   end   ]-------- ###
    ...
    try:
        ### --------[ changed ]-------- ###
        tojs_root = '%s/tojs' % cpp_root
        output_dir = '%s/auto' % cpp_root

        cmd_args = {
            'lolo.ini': ('lolo', 'lolo_auto')
        }
        ### --------[   end   ]-------- ###
